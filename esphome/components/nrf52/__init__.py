...
async def to_code(config: ConfigType) -> None:
    ...
    if config[KEY_BOOTLOADER] == BOOTLOADER_MCUBOOT:
        ...
        # config with no zephyr_mcumgr OTA still needs it, and without it the app
        # would link as a plain SoftDevice app at 0x27000 and MCUboot would never
        # build.
        ...
        if config[CONF_BOARD] == "xiao_ble":
            ...
            # labels, so we add them here.
            #
            # MCUboot boots from the reset vector at 0x0 and the Partition
            ...
            # image overlay.
            # MCUboot is hash-only (signature type "none") to fit its flash
            # budget. Under sysbuild the signature type is chosen at the sysbuild
            ...
            # slot0_partition (REQUIRED) to size the signed app image. Add the
            # same nodes to the app's own devicetree overlay.
